...
var (
	...
	BotId = ""
	// SquadweURL URL for Squadwe installation
	SquadweURL = ""
	// SquadweBotToken Squadwe Bot token from database
	SquadweBotToken = os.Getenv("squadweBotToken")
...
func handler(request events.APIGatewayProxyRequest) (events.APIGatewayProxyResponse, error) {
	...
	if req.MessageType == "incoming" || req.ContentAttributes.SubmittedValue != nil {
		...
		var squadweResponse *http.Response

		squadweChan := make(chan *http.Response)
		for _, response := range botMessages.Responses {
			//log.Printf("Bot responses: %v", response)
			if len(response.Text) > 0 {
				requestBody = SendToSquadwe(req.Conversation.Id, response, req.Conversation.Meta.Sender.CustomAttributes, response.Choices, response.Type)
			} else {
				requestBody = SendToSquadwe(req.Conversation.Id, response, req.Conversation.Meta.Sender.CustomAttributes, response.Choices, response.Type)

			}

			go SendPostAsync(requestBody, squadweChan)
			squadweResponse = <-squadweChan
			//log.Println(squadweResponse)
		}
		defer squadweResponse.Body.Close()
...
func SendToSquadwe(conversationId int, response Response, customAttributes CustomAttributes, choices []Choice, messageType string) *http.Request {
	var postBody []byte
	var err error
	...
	urlString := SquadweURL + "/api/v1/accounts/1/conversations/" + strconv.Itoa(conversationId) + "/messages"
	req, err := http.NewRequest(http.MethodPost, urlString, bytes.NewReader(postBody))
	req.Header.Add("Content-Type", "application/json")
	req.Header.Add("api_access_token", SquadweBotToken)
	if err != nil {
		log.Println("Error in sending to squadwe...")
